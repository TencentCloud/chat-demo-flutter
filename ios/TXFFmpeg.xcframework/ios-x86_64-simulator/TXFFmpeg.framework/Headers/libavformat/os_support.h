...
int liteav_ff_poll(struct pollfd *fds, nfds_t numfds, int timeout);
#define poll liteav_ff_poll
#endif /* HAVE_POLL_H */
...
static inline int win32_rename(const char *src_utf8, const char *dest_utf8)
{
    ...
    if (utf8towchar(dest_utf8, &dest_w)) {
        liteav_av_free(src_w);
        return -1;
    }
    if (!src_w || !dest_w) {
        liteav_av_free(src_w);
        liteav_av_free(dest_w);
        goto fallback;
    }
