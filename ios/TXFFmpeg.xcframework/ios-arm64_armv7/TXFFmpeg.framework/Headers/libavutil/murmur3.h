 ...
 * seed (liteav_av_murmur3_init_seeded()) and one that uses a fixed arbitrary integer
 * as the seed, and therefore does not (liteav_av_murmur3_init()).
 *
 ...
 */
struct AVMurMur3 *liteav_av_murmur3_alloc(void);
 ...
 *
 * @see liteav_av_murmur3_init()
 * @see @ref lavu_murmur3_seedinfo "Detailed description" on a discussion of
 * seeds for MurmurHash3.
 */
void liteav_av_murmur3_init_seeded(struct AVMurMur3 *c, uint64_t seed);

/**
 * Initialize or reinitialize an AVMurMur3 hash context.
 *
 * Equivalent to liteav_av_murmur3_init_seeded() with a built-in seed.
 *
 * @param[out] c    Hash context
 *
 * @see liteav_av_murmur3_init_seeded()
 * @see @ref lavu_murmur3_seedinfo "Detailed description" on a discussion of
 * seeds for MurmurHash3.
 */
void liteav_av_murmur3_init(struct AVMurMur3 *c);
...
void liteav_av_murmur3_update(struct AVMurMur3 *c, const uint8_t *src, int len);
#else
void liteav_av_murmur3_update(struct AVMurMur3 *c, const uint8_t *src, size_t len);
#endif
 ...
 */
void liteav_av_murmur3_final(struct AVMurMur3 *c, uint8_t dst[16]);
