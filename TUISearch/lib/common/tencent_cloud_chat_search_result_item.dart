  ...
  Widget build(BuildContext context) {
    List<TextSpan> buildHighlightedTitle(String title, String keyword, Color primaryColor) {
      ...
      while (startIndex < title.length) {
        ...
        spans.add(TextSpan(
            text: title.substring(keywordIndex, keywordIndex + keyword.length), style: TextStyle(color: primaryColor)));
    ...
    return TencentCloudChatThemeWidget(
        build: (context, colorTheme, textStyle) => ListTile(
              dense: true,
              contentPadding: EdgeInsets.symmetric(vertical: data.subTitle != null ? 0 : 4, horizontal: 16),
