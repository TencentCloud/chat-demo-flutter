...
class TencentCloudChatMessageSeparateDataProvider extends ChangeNotifier {
  ...
  sendImageMessage({
    ...
    String cacheImagePath = imagePath!;
    if (Platform.operatingSystem == "ohos")  {
      cacheImagePath = await copyFileToOhosAppCache(imagePath!);
    }
  ...
  sendVideoMessage({
    ...
    if (!TencentCloudChatPlatformAdapter().isWeb) {
      String tempSnapshotPath = "${CommonUtils.appCacheDir}/${Pertypath().basename(videoPath ?? "")}.jpeg";
      snapshotPath = await VideoThumbnailOhos.thumbnailFile(
        video: videoPath ?? "",
        thumbnailPath: tempSnapshotPath,
        imageFormat: ImageFormat.JPEG,
        maxHeight: 1280,
        quality: 100);
    }

    String cacheVideoPath = videoPath!;
    if (Platform.operatingSystem == "ohos")  {
      cacheVideoPath = await copyFileToOhosAppCache(videoPath!);
    }

    final messageInfo = await TencentCloudChat.instance.chatSDKInstance.messageSDK.createVideoMessage(
      videoFilePath: cacheVideoPath ?? "",
  ...
  sendFileMessage({
    ...
    if (Platform.operatingSystem == "ohos")  {
      cacheFilePath = await copyFileToOhosAppCache(filePath!);
    }
  ...
  sendVoiceMessage(String voicePath, int duration) async {
    ...
    String cacheVoicePath = voicePath!;
    if (Platform.operatingSystem == "ohos")  {
      cacheVoicePath = await copyFileToOhosAppCache(voicePath!);
    }

    final messageInfo = await TencentCloudChat.instance.chatSDKInstance.messageSDK.createVoiceMessage(
      voicePath: cacheVoicePath,
      duration: duration,
    );
    return _sendMessage(messageInfoResult: messageInfo);
  }
  ...
  Future<String> copyFileToOhosAppCache(String sourcePath) async {
    final targetPath = '${CommonUtils.appCacheDir.path}/${Pertypath().basename(sourcePath)}';
    try {
      final sourceFile = File(sourcePath);
      if (!await sourceFile.exists()) {
        throw Exception('source file not exist: $sourcePath');
      }
      ...
      debugPrint("copyFileToOhosAppCache error: ${e.toString()}");
    }
